#!/usr/bin/python3
'''Unit tests for the Base class and its functionalities.'''
import unittest
...
class TestBase(unittest.TestCase):
    ...
    def test_A_nb_objects_private(self):
        '''Verify that nb_objects is a private class attribute.'''
        self.assertTrue(hasattr(Base, "_Base__nb_objects"))

    def test_B_nb_objects_initialized(self):
        '''Check that nb_objects is initialized to zero.'''
    ...
    def test_D_constructor_args_2(self):
        '''Verify the constructor with two non-self arguments.'''
        with self.assertRaises(TypeError) as e:
    ...
    def test_E_consecutive_ids(self):
        '''Ensure that IDs are assigned consecutively.'''
        b1 = Base()
        b2 = Base()
        self.assertEqual(b1.id + 1, b2.id)

    def test_F_id_synced(self):
        '''Verify synchronization between class and instance IDs.'''
        b = Base()
        self.assertEqual(getattr(Base, "_Base__nb_objects"), b.id)

    def test_F_id_synced_more(self):
        '''Further check synchronization of IDs.'''
        b = Base()
        b = Base("Foo")
        b = Base(98)
        b = Base()
    ...
    def test_G_custom_id_int(self):
        '''Test the assignment of a custom integer ID.'''
        i = 98
        b = Base(i)
        self.assertEqual(b.id, i)

    def test_G_custom_id_str(self):
        '''Test the assignment of a custom string ID.'''
        i = "FooBar"
        b = Base(i)
        self.assertEqual(b.id, i)

    def test_G_id_keyword(self):
        '''Verify that ID can be passed as a keyword argument.'''
        i = 421
        b = Base(id=i)
        self.assertEqual(b.id, i)
    ...
    def test_H_to_json_string(self):
        '''Test the to_json_string() method signature and functionality.'''
        with self.assertRaises(TypeError) as e:
            Base.to_json_string()
    ...
    def test_H_test_from_json_string(self):
        '''Test the from_json_string() method signature and functionality.'''
        with self.assertRaises(TypeError) as e:
...
{"x": 101, "y": 20123, "width": 312321, "id": 522244, "height": 34340}]'
    ...
    def test_I_save_to_file(self):
        '''Test the save_to_file() method functionality.'''
        import os
        r1 = Rectangle(10, 7, 2, 8)
        r2 = Rectangle(2, 4)
    ...
    def test_J_create(self):
        '''Test the create() method functionality.'''
        r1 = Rectangle(3, 5, 1)
    ...
    def test_K_load_from_file(self):
        '''Test the load_from_file() method functionality.'''
        r1 = Rectangle(10, 7, 2, 8)
        r2 = Rectangle(2, 4)
        list_in = [r1, r2]
